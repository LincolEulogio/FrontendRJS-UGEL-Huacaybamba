      ...
      <section className="relative h-[450px] flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-6 text-center text-white">
          ...
          <p className="text-xl md:text-2xl text-white/90 max-w-3xl mx-auto leading-relaxed">
            Gestionamos eficientemente los recursos humanos, materiales y
            financieros para el óptimo funcionamiento institucional
          </p>
        </div>
      </section>
        ...
        <div className="grid md:grid-cols-2 gap-8">
          {objectives.map((obj, index) => (
            ...
            >
              ...
              <h3 className={`text-2xl font-bold text-${obj.color}-900 mb-3`}>
                {obj.title}
              </h3>
              <p className={`text-${obj.color}-700 leading-relaxed`}>
                {obj.description}
              </p>
            </div>
          ))}
      ...
      <section className="max-w-6xl mx-auto py-20 px-6">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {functions.map((func, index) => (
            <div
              key={index}
              className="bg-white rounded-[24px] p-8 shadow-lg hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 border border-slate-200"
            >
      ...
      <section className="max-w-6xl mx-auto py-20 px-6">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          {team.map((member) => (
            <div
              key={member.id}
              className="bg-white rounded-[24px] overflow-hidden shadow-lg hover:shadow-2xl hover:-translate-y-2 transition-all duration-300"
            >
              ...
              <div className="p-6">
                <div className="flex items-center gap-2 mb-2">
                  <span className="material-symbols-rounded text-[#1e40af] text-sm">
                    badge
                  </span>
                ...
                <p className="text-sm text-[#1e40af] font-semibold mb-2">
                  {member.role}
                </p>
