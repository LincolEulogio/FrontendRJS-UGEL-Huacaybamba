...
const LegalAdvisory = () => {
  ...
  const functions = [
    { icon: 'balance', title: 'Dictámenes y Opiniones', description: 'Emitir dictámenes y opiniones legales sobre recursos administrativos de su competencia técnica.' },
    { icon: 'forum', title: 'Consultas Jurídicas', description: 'Informar, opinar y absolver consultas de carácter jurídico-legal de la Unidad Ejecutora.' },
    { icon: 'description', title: 'Proyectos de Resolución', description: 'Formular proyectos de resolución, disposiciones, contratos y convenios de carácter legal.' },
    { icon: 'library_books', title: 'Legislación Educativa', description: 'Sistematizar y difundir la legislación educativa vigente para el conocimiento público.' }
  ];

  return (
    <div className="bg-background-light dark:bg-background-dark min-h-screen">
      <main className="pt-12 pb-24 px-6 max-w-[1920px] mx-auto">
        {/* Header */}
        <div className="text-center mb-16">
          ...
          <p className="max-w-2xl mx-auto text-lg text-slate-500 dark:text-slate-400 leading-relaxed">
            Brindamos soporte legal especializado para garantizar la legalidad de los actos administrativos y el correcto cumplimiento del marco normativo institucional.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-6 gap-6">
              ...
              <ul className="space-y-6">
                {objectives.map((objective, index) => (
                  <li key={index} className="flex gap-4">
                    <span className="material-symbols-rounded text-primary dark:text-blue-400">check_circle</span>
          ...
          <div className="md:col-span-2 lg:col-span-2 glass overflow-hidden rounded-2xl flex flex-col group">
            <div className="relative h-2/3">
              ...
              <div className="absolute bottom-6 left-6 text-white">
                <p className="text-sm font-medium opacity-80 uppercase tracking-widest">Director de Área</p>
            ...
            <div className="p-6 bg-white dark:bg-slate-900/50 flex-grow flex flex-col justify-between">
              <div className="space-y-3">
                <div className="flex items-center gap-3 text-sm">
                  <span className="material-symbols-rounded text-primary text-lg">badge</span>
                  <span className="text-slate-600 dark:text-slate-400">Cód: 448-03-01-5</span>
                </div>
                <div className="flex items-center gap-3 text-sm">
                  <span className="material-symbols-rounded text-primary text-lg">history_edu</span>
                  <span className="text-slate-600 dark:text-slate-400">Colegiado habilitado</span>
                </div>
              </div>
              <button className="mt-6 w-full py-3 bg-slate-100 dark:bg-slate-800 rounded-xl font-bold text-sm flex items-center justify-center gap-2 group-hover:bg-primary group-hover:text-white transition-all">
                <span className="material-symbols-rounded text-sm">mail</span> CONTACTAR AHORA
              </button>
            </div>
          </div>
          ...
          <div className="md:col-span-1 glass p-6 rounded-2xl bg-primary text-white border-none flex flex-col justify-center items-center text-center">
            ...
            <span className="text-xs font-bold uppercase tracking-widest opacity-80">N° de Orden</span>
          </div>
              ...
              {functions.map((func, index) => (
                <div key={index} className="space-y-2">
                  <h4 className="font-bold text-primary dark:text-blue-400 flex items-center gap-2">
                    <span className="material-symbols-rounded text-sm">{func.icon}</span> {func.title}
                  </h4>
                  <p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed">{func.description}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="md:col-span-2 lg:col-span-2 glass overflow-hidden rounded-2xl relative">
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="bg-white/90 dark:bg-slate-900/90 p-4 rounded-xl shadow-2xl text-center">
                <p className="font-bold text-primary dark:text-white">Transparencia Legal</p>
              </div>
            </div>
          </div>
              ...
              <table className="w-full text-left">
                <thead>
                  <tr className="border-b border-slate-200 dark:border-slate-700">
                    <th className="pb-4 font-bold text-slate-400 uppercase text-xs tracking-wider">N° Orden</th>
                    <th className="pb-4 font-bold text-slate-400 uppercase text-xs tracking-wider">Cargo Estructural</th>
                    <th className="pb-4 font-bold text-slate-400 uppercase text-xs tracking-wider">Código</th>
                    <th className="pb-4 font-bold text-slate-400 uppercase text-xs tracking-wider text-right">Total</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                  <tr className="group hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                    ...
                    <td className="py-5 font-mono text-sm opacity-70">448-03-01-5</td>
                    <td className="py-5 text-right font-bold">1</td>
                  </tr>
                </tbody>
                <tfoot>
                  <tr className="bg-slate-50/50 dark:bg-slate-800/50 font-black">
                    <td className="py-4 px-4 text-right" colSpan="3">TOTAL CARGOS</td>
